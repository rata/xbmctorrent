...
VIDEO_PLAYABLE_POLL = 5000
PLAYING_EVENT_INTERVAL = 60
MIN_DOWNLOAD_MB = float(plugin.get_setting("min_download_size"))
...
MPLAYER_CMD = ["mplayer", "-v", "-vo", "null", "-vc", "null", "-ao", "null",
		"-frames", "0", "-identify", "-noconsolecontrols", "-noconfig", "all",
		"-cache-min", "0", "-prefer-ipv4"]



def run_cmd(cmd_list, url, timeout=10):
        """
	Runs "cmd_list" and if in timeout seconds it didn't end, it kills
	(SIGKILL) it
	"""
	import subprocess

        # XXX: the command "timeout" is available coreutils (recent versions)
        cmd = ["timeout", "-s", "KILL", str(timeout)] + cmd_list + [url]

        proc = subprocess.Popen(cmd, shell=False, stdout=subprocess.PIPE,
                                stderr=subprocess.STDOUT)

        # Save all the output (stdout and stderr)
...
class TorrentPlayer(xbmc.Player):
    ...
    def onPlayBackStarted(self):
        # Ask to download subtitles
        subs_enabled = xbmc.getCondVisibility("VideoPlayer.HasSubtitles")
        if plugin.get_setting("auto_subs", bool) and not subs_enabled and not self.asked_subs:
            self.asked_subs = True
            xbmc.sleep(1000)

            # The first one only works in XBMC 13. The seconds one seems to only
            # work on Frodo. So run both, only one of them will work.
            ...
            with closing(SafeDialogProgress(delay_create=0)) as dialog:
                ...
                while not has_resolved:
                    ...
                    if status["state"] >= 3 and not has_resolved: # Downloading?
                        ...
                        resolved_url = "http://%s/files/%s" % (t2h_instance.bind_address, urllib.quote(biggest_file["name"]))

                        if not (mb_complete > MIN_DOWNLOAD_MB or bytes_complete >= biggest_file["size"]):
                            continue

                        ret_code, _ = run_cmd(MPLAYER_CMD, resolved_url)
                        if ret_code != 0:
                            xbmc.sleep(VIDEO_PLAYABLE_POLL)
                            continue

                        plugin.log.info("Resolving to %s" % resolved_url)
                        has_resolved = True
                        item = {
                            "path": resolved_url,
                        }
